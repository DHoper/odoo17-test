...
class AccountMove(models.Model):
    ...
    asset_depreciation_ids = fields.One2many(
        ...
        "move_id",
        string="Asssets Depreciation Lines",
    )
...
class AccountInvoiceLine(models.Model):
    ...
    asset_category_id = fields.Many2one(
        "account.asset.category", string="Assest Category"
    )
    asset_start_date = fields.Date(
        string="Assest Start Date", compute="_get_asset_date", readonly=True, store=True
    )
    asset_end_date = fields.Date(
        string="Assest End Date", compute="_get_asset_date", readonly=True, store=True
    )
    asset_mrr = fields.Float(
        string="Monthlys Recurring Revenue",
