...
class ImLivechatChannel(models.Model):
    ...
    def _get_livechat_discuss_channel_vals(
        ...
        user_operator = False
        if chatbot_script:
            if chatbot_script.id not in self.browse(self.ids).mapped(
                "rule_ids.chatbot_script_id.id"
            ):
                return False
        else:
        ...
        if user_id:
            ...
            if (
                ...
                and user_operator
                and visitor_user != user_operator
    ...
    def _get_less_active_operator(self, operator_statuses, operators):
        ...
        active_operator_ids = set(
            map(lambda s: s["livechat_operator_id"], operator_statuses)
        )
        inactive_operators = [
            operator
            for operator in operators
            if operator.partner_id.id not in active_operator_ids
        ]
        ...
        i = 1
        while (
            i < len(operator_statuses)
            and operator_statuses[i]["in_call"] == in_call
            and operator_statuses[i]["count"] == count
        ):
            i += 1
        less_active_operator_id = random.choice(operator_statuses[:i])[
        ...
        return next(
            operator
            for operator in operators
            if operator.partner_id.id == less_active_operator_id
        )
    ...
    def _get_operator(self, previous_operator_id=None, lang=None, country_id=None):
        ...
        if previous_operator_id in self.available_operator_ids.partner_id.ids:
            previous_operator_status = next(
                (
                    ...
                    if status["livechat_operator_id"] == previous_operator_id
                ),
                None,
            )
            if (
        ...
        if lang:
            ...
            if same_lang_operator_ids:
                operator = self._get_less_active_operator(
                    operator_statuses, same_lang_operator_ids
                )
            else:
                addition_lang_operator_ids = self.available_operator_ids.filtered(
                    lambda operator: lang
                    in operator.res_users_settings_id.livechat_lang_ids.mapped("code")
                )
                if addition_lang_operator_ids:
                    operator = self._get_less_active_operator(
                        operator_statuses, addition_lang_operator_ids
                    )
        ...
        if country_id and not operator:
            ...
            if same_country_operator_ids:
                operator = self._get_less_active_operator(
                    operator_statuses, same_country_operator_ids
                )
        # Try to get a random operator, regardless of the lang or the country
        if not operator:
            operator = self._get_less_active_operator(
                operator_statuses, self.available_operator_ids
            )
        return operator
    ...
    def get_livechat_info(self, username=None):
        ...
        info = {}
        info["available"] = (
            self.chatbot_script_count or len(self.available_operator_ids) > 0
        )
