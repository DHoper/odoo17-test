...
class Channel(models.Model):
        ...
        for channel in self:
            if channel.channel_type == "channel":
                channel.is_editable = (
                    ...
                    or channel.create_uid.id == self.env.user.id
                )
    ...
    def _unlink_except_all_employee_channel(self):
        ...
        self.env["bus.bus"]._sendmany(
            [
                (channel, "discuss.channel/delete", {"id": channel.id})
                for channel in self
            ]
        )
                ...
                if guest:
                    notifications.append(
                        (
                            ...
                            "discuss.channel/joined",
                            {
                                "channel": member.channel_id.with_context(
                                    guest=guest
                                )._channel_info()[0],
                            },
                        )
                        ...
                        "Thread": {
                            "channelMembers": [
                                (
                                    "ADD",
                                    list(
                                        new_members._discuss_channel_member_format().values()
                                    ),
                                )
                            ],
                            "id": channel.id,
                            ...
                            "Thread": {
                                "channelMembers": [
                                    (
                                        "ADD",
                                        list(
                                            existing_members._discuss_channel_member_format().values()
                                        ),
                                    )
                                ],
                                "id": channel.id,
    ...
    def _rtc_cancel_invitations(self, member_ids=None):
        ...
        if members:
            ...
            self.env["bus.bus"]._sendone(
                self, "mail.record/insert", {"Thread": channel_data}
            )
        return channel_data
    ...
    def _notify_get_recipients(self, message, msg_vals, **kwargs):
        ...
        message_type = (
            msg_vals.get("message_type", "comment")
            if msg_vals
        ...
        for partner in self.env["res.partner"].browse(partner_ids):
            ...
            if user_id:
                ...
                for channel_info in user_channels._channel_info():
                    notifications.append(
                        (partner, "mail.record/insert", {"Thread": channel_info})
                    )
    ...
    def _find_or_create_persona_for_channel(
        self, guest_name, timezone, country_code, post_joined_message=True
    ):
        """
        ...
        if not self.env.user._is_public():
            self.add_members(
                [self.env.user.partner_id.id], post_joined_message=post_joined_message
            )
        else:
            ...
            self.add_members(
                guest_ids=guest.ids, post_joined_message=post_joined_message
            )
    ...
    def _channel_info(self):
        ...
        rtc_sessions_by_channel = (
            self.sudo().rtc_session_ids._mail_rtc_session_format_by_channel(extra=True)
        )
        current_partner, current_guest = self.env["res.partner"]._get_current_persona()
        ...
        """,
            {
                ...
                "current_partner_id": current_partner.id or None,
                "current_guest_id": current_guest.id or None,
            },
        )
        all_needed_members = self.env["discuss.channel.member"].browse(
            [m["id"] for m in self.env.cr.dictfetchall()]
        )
        ...
        invited_members_by_channel = defaultdict(
            lambda: self.env["discuss.channel.member"]
        )
        ...
        for channel in self:
            ...
            if current_partner or current_guest:
                ...
                member = member_of_current_user_by_channel.get(
                    channel, self.env["discuss.channel.member"]
                ).with_prefetch(
                    [m.id for m in member_of_current_user_by_channel.values()]
                )
                if member:
                    info["channelMembers"] = [
                        ("ADD", list(member._discuss_channel_member_format().values()))
                    ]
                    ...
                    info["mute_until_dt"] = (
                        member.mute_until_dt.strftime(DEFAULT_SERVER_DATETIME_FORMAT)
                        if member.mute_until_dt
                        else False
                    )
            ...
            if channel.channel_type != "channel":
                ...
                info["seen_partners_info"] = sorted(
                    [
                        {
                            "id": cm.id,
                            "partner_id" if cm.partner_id else "guest_id": (
                                cm.partner_id.id if cm.partner_id else cm.guest_id.id
                            ),
            ...
            channel = self.create(
                {
                    ...
                    "channel_type": "chat",
                    "name": ", ".join(
                        self.env["res.partner"].browse(partners_to).mapped("name")
                    ),
                }
            )
    ...
    def _set_last_seen_message(self, last_message, allow_older=False):
            ...
            [
                ...
                [
                    (
                        ...
                        if current_partner
                        else ("guest_id", "=", current_guest.id)
                    )
                ],
                (
                    []
        ...
        member.write(
            {
                "fetched_message_id": max(
                    member.fetched_message_id.id, last_message.id
                ),
                "seen_message_id": last_message.id,
        ...
        data["partner_id" if current_partner else "guest_id"] = (
            current_partner.id if current_partner else current_guest.id
        )
    ...
    def channel_create(self, name, group_id):
        ...
        self.env["bus.bus"]._sendone(
            self.env.user.partner_id, "mail.record/insert", {"Thread": channel_info}
        )
        return new_channel
    ...
    def load_more_members(self, known_member_ids):
        ...
        return {
            "channelMembers": [
                ("ADD", list(unknown_members._discuss_channel_member_format().values()))
            ],
            "memberCount": count,
        }
    ...
    def execute_command_help(self, **kwargs):
        ...
        if self.channel_type == "channel":
            ...
            all_channel_members = self.env["discuss.channel.member"].with_context(
                active_test=False
            )
    ...
    def _notify_thread_by_web_push(
        ...
        if chat_channels:
            ...
            channel_rdata += [
                ...
                for partner in chat_channels.channel_member_ids.filtered(
    ...
    def _notify_by_web_push_prepare_payload(self, message, msg_vals=False):
        ...
        record_name = (
            ...
            if msg_vals and "record_name" in msg_vals
        ...
        if self.channel_type == "chat":
            ...
            payload["options"]["icon"] = "/discuss/channel/%d/partner/%d/avatar_128" % (
                message.res_id,
                author_id[0],
            )
