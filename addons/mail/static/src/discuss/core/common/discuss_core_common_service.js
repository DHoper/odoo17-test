    ...
    setup() {
        this.messagingService.isReady.then(data => {
            ...
            this.busService.subscribe("discuss.channel/joined", payload => {
                const { channel, invited_by_user_id: invitedByUserId } =
                    payload;
                ...
                if (
                    invitedByUserId &&
                    invitedByUserId !== this.store.user?.user?.id
                ) {
            ...
            this.busService.subscribe("discuss.channel/leave", payload => {
                ...
                this.notificationService.add(
                    _t("You unsubscribed from %s.", thread.displayName),
                    {
                        type: "info",
                    }
                );
            ...
            this.busService.addEventListener(
                ...
                ({ detail: notifications }) => {
                    ...
                    const channelsLeft = new Set(
                        notifications
                            .filter(
                                ({ type }) => type === "discuss.channel/leave"
                            )
            ...
            this.busService.subscribe(
                ...
                payload => {
                    ...
                    const lastMessageId =
                        this.messageService.getLastMessageId();
            ...
            this.busService.subscribe(
                ...
                payload => {
                    const { channel_id, id, last_message_id, partner_id } =
                        payload;
            ...
            this.env.bus.addEventListener(
                ...
                ({ detail: { message } }) => {
                    if (message.originThread) {
                        if (
                            message.id > message.originThread.seen_message_id
                        ) {
...
export const discussCoreCommon = {
     ...
     */
    start(env, services) {
        const discussCoreCommon = reactive(
            new DiscussCoreCommon(env, services)
        );
