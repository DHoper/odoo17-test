...
QUnit.test("chat window does not fetch messages if hidden", async assert => {
    ...
    const [channeId1, channelId2, channelId3] = pyEnv[
        "discuss.channel"
    ].create([
        {
            channel_member_ids: [
...
QUnit.test(
    "click on hidden chat window should fetch its messages",
    async assert => {
        const pyEnv = await startServer();
        const [channeId1, channelId2, channelId3] = pyEnv[
            "discuss.channel"
        ...
        await click(
            ".o-mail-ChatWindowHiddenMenu-item .o-mail-ChatWindow-command[title='Open']"
        );
        await contains(".o-mail-Message-content", { text: "Orange" });
...
QUnit.test(
    ...
    async assert => {
        ...
        await contains(":nth-child(1 of .o-mail-ChatWindow)", {
            text: "channel-A",
        });
        await contains(":nth-child(2 of .o-mail-ChatWindow)", {
            text: "channel-D",
        });
