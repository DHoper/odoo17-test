...
class AccountAccount(models.Model):
    ...
    root_id = fields.Many2one(
        "account.root", compute="_compute_account_root", store=True, precompute=True
    )
    ...
    def _check_company_consistency(self):
        for company, accounts in tools.groupby(
            ...
            while self.env["account.account"].search(
                [
                    *self.env["account.account"]._check_company_domain(
                        default.get("company_id", False) or self.company_id
                    ),
                    ("code", "=", default["code"]),
                ],
                limit=1,
    ...
    def create(self, vals_list):
        ...
        for vals in vals_list:
            if "prefix" in vals:
                ...
                vals["code"] = self._search_new_account_code(
                    company, vals.pop("code_digits"), vals.pop("prefix"), cache
                )
...
class AccountGroup(models.Model):
    ...
    code_prefix_end = fields.Char(
        compute="_compute_code_prefix_end", readonly=False, store=True, precompute=True
    )
    company_id = fields.Many2one(
        "res.company",
        required=True,
    ...
    def _adapt_accounts_for_account_groups(self, account_ids=None):
        ...
        if self.env.context.get("delay_account_group_sync"):
            return
        company_ids = (
            account_ids.company_id.root_id.ids if account_ids else self.company_id.ids
        )
