...
class AccountChartTemplate(models.AbstractModel):
    ...
    def _get_demo_data_products(self, company=False):
        prod_templates = self.env["product.product"].search(
    ...
    def _get_demo_data_move(self, company=False):
        ...
        return {
            ...
            "demo_move_auto_reconcile_6": {
                ...
                "partner_id": "base.res_partner_2",
                "invoice_user_id": False,
                "invoice_date": (fields.Date.today() + timedelta(days=-1)).strftime(
                    "%Y-%m-%d"
                ),
                "delivery_date": (fields.Date.today() + timedelta(days=-1)).strftime(
                    "%Y-%m-%d"
                ),
                "invoice_line_ids": [
                    Command.create(
